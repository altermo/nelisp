...
F.all_completions={'all-completions',2,4,0,[[Search for partial matches of STRING in COLLECTION.
    ...
    while true do
        ...
        elseif typ==2 then
            if not lisp.eq(bucket,zero) then
                ...
                elt=bucket
                eltstring=elt
                if (bucket --[[@as nelisp._symbol]]).next then
                    bucket=(bucket --[[@as nelisp._symbol]]).next
                else
                    bucket=zero
                end
        ...
        if lisp.stringp(eltstring)
            and lisp.schars(str)<=lisp.schars(eltstring)
            and (lisp.nilp(hide_spaces) or
            (lisp.sbytes(str)>0 and lisp.sref(str,0)==b' ') or
            lisp.sref(eltstring,0)~=b' ') and
            (function ()
            local tem=vars.F.compare_strings(eltstring,zero,lisp.make_fixnum(lisp.schars(str)),
                str,zero,lisp.make_fixnum(lisp.schars(str)),vars.V.completion_ignore_case)
                return lisp.eq(tem,vars.Qt)
            end)()
        then
            if not match_regexps(eltstring,vars.V.completion_regexp_list,not lisp.nilp(vars.V.completion_ignore_case)) then
                goto continue
            end
...
properties.]])
    ...
    vars.defvar_bool('completion_ignore_case','completion-ignore-case',[[Non-nil means don't consider case significant in completion.
...
controls the behavior, rather than this variable.]])
    ...
    vars.defvar_lisp('completion_regexp_list','completion-regexp-list',[[List of regexps that should restrict possible completions.
