...
---@type nelisp.F
local F={}
F.frame_or_buffer_changed_p={'frame-or-buffer-changed-p',0,1,0,[[Return non-nil if the frame and buffer state appears to have changed.
...
function F.frame_or_buffer_changed_p.f(variable)
    ...
    for _,frame in ipairs(framelist) do
        ...
        if idx==lisp.asize(state) then
            goto changed
        elseif lisp.aref(state,idx)~=nvim.frame_name(frame --[[@as nelisp._frame]]) then
            goto changed
        end
        idx=idx+1
    end
    for _,buf in ipairs(buflist) do
        if lisp.sdata(nvim.bvar(buf--[[@as nelisp._buffer]],'name')):sub(1,1)==' ' then
            goto continue
        end
        if idx==lisp.asize(state) then
            goto changed
        ...
        if idx==lisp.asize(state) then
            goto changed
        elseif lisp.aref(state,idx)~=nvim.bvar(buf--[[@as nelisp._buffer]],'read_only') then
            goto changed
        end
        idx=idx+1
        if idx==lisp.asize(state) then
    ...
    if not lisp.vectorp(state)
        or n>lisp.asize(state)
        or (n+20)<(lisp.asize(state)/2) then
    ...
    for _,frame in ipairs(framelist) do
        ...
        idx=idx+1
        lisp.aset(state,idx,nvim.frame_name(frame --[[@as nelisp._frame]]))
        idx=idx+1
    end
    for _,buf in ipairs(buflist) do
        if lisp.sdata(nvim.bvar(buf--[[@as nelisp._buffer]],'name')):sub(1,1)==' ' then
        ...
        idx=idx+1
        lisp.aset(state,idx,nvim.bvar(buf--[[@as nelisp._buffer]],'read_only'))
        idx=idx+1
