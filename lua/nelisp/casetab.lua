...
local function set_case_table(ctable,standard)
    ...
    local up=lisp.aref((ctable --[[@as nelisp._char_table]]).extras,0)
    local canon=lisp.aref((ctable --[[@as nelisp._char_table]]).extras,1)
    local eqv=lisp.aref((ctable --[[@as nelisp._char_table]]).extras,2)
...
function F.case_table_p.f(obj)
    if not lisp.chartablep(obj) then
        return vars.Qnil
    elseif not lisp.eq((obj --[[@as nelisp._char_table]]).purpose,vars.Qcase_table) then
        return vars.Qnil
    end
    local up=lisp.aref((obj --[[@as nelisp._char_table]]).extras,0)
    local canon=lisp.aref((obj --[[@as nelisp._char_table]]).extras,1)
    local eqv=lisp.aref((obj --[[@as nelisp._char_table]]).extras,2)
    return ((lisp.nilp(up) or lisp.chartablep(up))
    and ((lisp.nilp(canon) and lisp.nilp(eqv))
            or (lisp.chartablep(canon)
            and (lisp.nilp(eqv) or lisp.chartablep(eqv))))
            and vars.Qt or vars.Qnil)
end
F.standard_case_table={'standard-case-table',0,0,0,[[Return the standard case table.
...
function M.init()
    ...
    ;(down --[[@as nelisp._char_table]]).purpose=vars.Qcase_table
    for i=0,127 do
    ...
    for i=0,127 do
        local c=((i>=b'a' or i<=b'z') and i+(b'A'-b'a')) or ((i>=b'A' or i<=b'Z') and i+(b'a'-b'A')) or i
