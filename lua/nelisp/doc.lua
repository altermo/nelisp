...
F.text_quoting_style={'text-quoting-style',0,0,0,[[Return the current effective text quoting style.
...
function F.text_quoting_style.f()
    if (lisp.nilp(vars.V.text_quoting_style) and default_to_grave_quoting_style())
        or lisp.eq(vars.V.text_quoting_style,vars.Qgrave) then
        return vars.Qgrave
...
function M.init_syms()
    ...
    vars.defvar_lisp('text_quoting_style','text-quoting-style',[[Style to use for single quotes in help and messages.
