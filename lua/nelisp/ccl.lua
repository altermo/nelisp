...
local function resolve_symbol_ccl_program(ccl)
    ...
    for i=0,veclen-1 do
        ...
        if lisp.fixnump(contents) and -0x80000000<=lisp.fixnum(contents) and lisp.fixnum(contents)<=0x7fffffff then
    ...
    if not (0<=lisp.fixnum(lisp.aref(result,ccl_header_buf_mag)) and
        ascending_order(0,lisp.fixnum(lisp.aref(result,ccl_header_eof)),lisp.asize(ccl))) then
        return vars.Qnil
    end
    return (unresolved and vars.Qt or result)
end
F.register_ccl_program={'register-ccl-program',2,2,0,[[Register CCL program CCL-PROG as NAME in `ccl-program-table'.
...
function M.init_syms()
    ...
    vars.defvar_lisp('font_ccl_encoder_alist','font-ccl-encoder-alist',[[Alist of fontname patterns vs corresponding CCL program.
