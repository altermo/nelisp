...
#endif
  return (
    DBL_MIN_EXP - 1 <= exponent && exponent < INT_MAX
      ? DBL_MANT_DIG - 1 - exponent
      : (DBL_MANT_DIG - DBL_MIN_EXP + (isnan (d) ? 2 : exponent == INT_MAX)));
}
...
static Lisp_Object
rounding_driver (Lisp_Object n, Lisp_Object d, double (*double_round) (double),
                 ...
                 EMACS_INT (*fixnum_divide) (EMACS_INT, EMACS_INT))
{
  ...
  int_divide (mpz[0], *rescale_for_division (n, &mpz[0], nscale, dscale),
