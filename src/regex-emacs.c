...
// NOTE:
// This file has ben (mostly) copy pasted all comments included.
...
#define RE_STRING_CHAR(p, multibyte) (multibyte ? STRING_CHAR (p) : *(p))
...
#define ISBLANK(c) \
  (IS_REAL_ASCII (c) ? ((c) == ' ' || (c) == '\t') : blankp (c))

#define ISGRAPH(c)                                                     \
  (SINGLE_BYTE_CHAR_P (c) ? (c) > ' ' && !((c) >= 0177 && (c) <= 0240) \
                          : graphicp (c))

#define ISPRINT(c)                                                      \
  (SINGLE_BYTE_CHAR_P (c) ? (c) >= ' ' && !((c) >= 0177 && (c) <= 0237) \
                          : printablep (c))
...
#define REGEX_USE_SAFE_ALLOCA \
  USE_SAFE_ALLOCA;            \
  sa_avail = emacs_re_safe_alloca

/* Assumes a 'char *destination' variable.  */
#define REGEX_REALLOCATE(source, osize, nsize) \
  (destination = SAFE_ALLOCA (nsize), memcpy (destination, source, osize))
...
#define CHARSET_RANGE_TABLE_BITS(p) \
  ((p)[2 + CHARSET_BITMAP_SIZE (p)] + (p)[3 + CHARSET_BITMAP_SIZE (p)] * 0x100)
  ...
  while (p < pend)
    {
      ...
      switch ((re_opcode_t) *p++)
        ...
        case on_failure_jump_nastyloop:
          EXTRACT_NUMBER_AND_INCR (mcnt, p);
          fprintf (dest, "/on_failure_jump_nastyloop to %td", p + mcnt - start);
          break;

        case on_failure_jump_loop:
          EXTRACT_NUMBER_AND_INCR (mcnt, p);
          fprintf (dest, "/on_failure_jump_loop to %td", p + mcnt - start);
          break;

        case on_failure_jump_smart:
          EXTRACT_NUMBER_AND_INCR (mcnt, p);
          fprintf (dest, "/on_failure_jump_smart to %td", p + mcnt - start);
          break;
        ...
        case succeed_n:
          ...
          fprintf (dest, "/succeed_n to %td, %d times", p - 2 + mcnt - start,
                   mcnt2);
          break;
        ...
        case jump_n:
          EXTRACT_NUMBER_AND_INCR (mcnt, p);
          EXTRACT_NUMBER_AND_INCR (mcnt2, p);
          fprintf (dest, "/jump_n to %td, %d times", p - 2 + mcnt - start,
                   mcnt2);
          break;
...
# define DEBUG_PRINT_COMPILED_PATTERN(p, s, e) \
   if (regex_emacs_debug > 0)                  \
   print_partial_compiled_pattern (stderr, s, e)
...
static bool analyze_first (struct re_pattern_buffer *bufp, re_char *p,
                           re_char *pend, char *fastmap);
...
#define EXTEND_RANGE_TABLE(work_area, n)             \
  do                                                 \
    {                                                \
      if (((work_area).used + (n)) * sizeof (int)    \
          > (unsigned long) (work_area).allocated)   \
        {                                            \
      ...
      for (C0 = (FROM); C0 <= (TO); C0++)                                \
        {                                                                \
          ...
          if (CHAR_BYTE8_P (C1))                                         \
            ...
            {                                                            \
              C2 = TRANSLATE (C1);                                       \
              if (C2 == C1 || (C1 = RE_CHAR_TO_UNIBYTE (C2)) < 0)        \
                C1 = C0;                                                 \
              SET_LIST_BIT (C1);                                         \
              for (I = RANGE_TABLE_WORK_USED (work_area) - 2; I >= USED; \
                   I -= 2)                                               \
                {                                                        \
...
re_wctype_parse (const unsigned char **strp, ptrdiff_t limit)
{
  ...
  *strp = (const unsigned char *) (it + 2);
  ...
  switch (it - beg)
    {
    case 4:
      if (!memcmp (beg, "word", 4))
        return RECC_WORD;
      break;
    case 5:
      if (!memcmp (beg, "alnum", 5))
        return RECC_ALNUM;
      if (!memcmp (beg, "alpha", 5))
        return RECC_ALPHA;
      if (!memcmp (beg, "space", 5))
        return RECC_SPACE;
      if (!memcmp (beg, "digit", 5))
        return RECC_DIGIT;
      if (!memcmp (beg, "blank", 5))
        return RECC_BLANK;
      if (!memcmp (beg, "upper", 5))
        return RECC_UPPER;
      if (!memcmp (beg, "lower", 5))
        return RECC_LOWER;
      if (!memcmp (beg, "punct", 5))
        return RECC_PUNCT;
      if (!memcmp (beg, "ascii", 5))
        return RECC_ASCII;
      if (!memcmp (beg, "graph", 5))
        return RECC_GRAPH;
      if (!memcmp (beg, "print", 5))
        return RECC_PRINT;
      if (!memcmp (beg, "cntrl", 5))
        return RECC_CNTRL;
      break;
    case 6:
      if (!memcmp (beg, "xdigit", 6))
        return RECC_XDIGIT;
      break;
    case 7:
      if (!memcmp (beg, "unibyte", 7))
        return RECC_UNIBYTE;
      break;
    case 8:
      if (!memcmp (beg, "nonascii", 8))
        return RECC_NONASCII;
      break;
    case 9:
      if (!memcmp (beg, "multibyte", 9))
...
regex_compile (re_char *pattern, ptrdiff_t size, bool posix_backtracking,
               const char *whitespace_regexp, struct re_pattern_buffer *bufp)
{
  ...
  compile_stack.stack
    = xmalloc (INIT_COMPILE_STACK_SIZE * sizeof *compile_stack.stack);
                ...
                if (p < pend && p[0] == '-' && p[1] != ']')
                  {
                    ...
                    if (CHAR_BYTE8_P (c1) && !ASCII_CHAR_P (c)
                        && !CHAR_BYTE8_P (c))
            ...
            case '(':
              {
                ...
                if (COMPILE_STACK_FULL)
                  compile_stack.stack
                    = xpalloc (compile_stack.stack, &compile_stack.size, 1, -1,
                  ...
                  { /* If the upper bound is > 1, we need to insert
                    ...
                    if (lower_bound == 0)
                    ...
                    else
                      {
                        ...
                        INSERT_JUMP2 (succeed_n, laststart, b + 5 + nbytes,
                                      lower_bound);
                        b += 5;
...
group_in_compile_stack (compile_stack_type compile_stack, regnum_t regnum)
{
  ...
  for (this_element = compile_stack.avail - 1; this_element >= 0;
       this_element--)
...
static bool
forall_firstchar_1 (re_char *p, re_char *pend, re_char *loop_beg,
                    re_char *loop_end, bool f (const re_char *p, void *arg),
                    void *arg)
{
  eassert (p >= loop_beg);
  eassert (p <= loop_end);
...
#if ENABLE_CHECKING
                    fprintf (stderr,
                             "FORALL_FIRSTCHAR: Broken assumption3!!\n");
#endif
                    return false;
                  }
                  ...
                  bool f (re_char *p, void *arg), void *arg)
{
  ...
  return forall_firstchar_1 (p, pend, bufp ? bufp->buffer - 1 : p,
                             bufp ? bufp->buffer + bufp->used + 1 : pend, f,
                             arg);
}
...
analyze_first_fastmap (const re_char *p, void *arg)
{
  ...
  switch (*p)
    ...
    case charset_not:
      {
        ...
        for (j = CHARSET_BITMAP_SIZE (p) * BYTEWIDTH; j < (1 << BYTEWIDTH); j++)
          data->fastmap[j] = 1;
      }
      FALLTHROUGH;
    case charset:
    ...
    case notcategoryspec:
      ...
      if (!data->match_any_multibyte_characters)
        {
          for (j = MIN_MULTIBYTE_LEADING_CODE; j <= MAX_MULTIBYTE_LEADING_CODE;
               j++)
            data->fastmap[j] = 1;
          data->match_any_multibyte_characters = true;
        }
...
static bool
analyze_first (struct re_pattern_buffer *bufp, re_char *p, re_char *pend,
               char *fastmap)
{
  eassert (pend);
  struct anafirst_data data
    = { bufp ? bufp->multibyte : false, fastmap, false };
  bool safe
    = forall_firstchar (bufp->used ? bufp : NULL, p, pend,
                        fastmap ? analyze_first_fastmap : analyze_first_null,
                        &data);
  return !safe;
}
...
re_compile_fastmap (struct re_pattern_buffer *bufp)
{
  ...
  bufp->can_be_null
    = analyze_first (bufp, bufp->buffer, bufp->buffer + bufp->used, fastmap);
} /* re_compile_fastmap */
...
re_search_2 (struct re_pattern_buffer *bufp, const char *str1, ptrdiff_t size1,
             const char *str2, ptrdiff_t size2, ptrdiff_t startpos,
             ptrdiff_t range, struct re_registers *regs, ptrdiff_t stop)
{
  ptrdiff_t val;
  ...
  if (bufp->used > 0 && (re_opcode_t) bufp->buffer[0] == at_dot && range > 0)
    {
      TODO; // range = PT_BYTE - BEGV_BYTE - startpos;
      if (range < 0)
        return -1;
    }
  ...
  for (;;)
    {
      ...
      val = re_match_2_internal (bufp, string1, size1, string2, size2, startpos,
                                 regs, stop);
...
static bool bcmp_translate (re_char *, re_char *, ptrdiff_t, Lisp_Object, bool);

/* This converts PTR, a pointer into one of the search strings 'string1'
   and 'string2' into an offset from the beginning of that string.  */
#define POINTER_TO_OFFSET(ptr) \
  (FIRST_STRING_P (ptr) ? (ptr) - string1 : (ptr) - string2 + (ptrdiff_t) size1)
                 ...
                 Lisp_Object canon_table)
{
  ...
  if (unibyte && c < (1 << BYTEWIDTH))
    { /* Lookup bitmap.  */
      ...
      if ((unsigned) c < (unsigned) (CHARSET_BITMAP_SIZE (p) * BYTEWIDTH)
  ...
  else if (rtp)
    {
      ...
      /* Sort tests by the most commonly used classes with some adjustment to
         which tests are easiest to perform.  Take a look at comment in
         re_wctype_parse for table with frequencies of character class names. */
                           ...
                           re_char *p2)
{
  ...
  else if ((re_opcode_t) *p1 == charset || (re_opcode_t) *p1 == charset_not)
    {
      if (!execute_charset (&p1, c, c, !multibyte || ASCII_CHAR_P (c), Qnil))
        {
          DEBUG_PRINT ("	 No match => fast loop.\n");
          return true;
        }
    }
  else if ((re_opcode_t) *p1 == anychar && c == '\n')
    {
      DEBUG_PRINT ("   . != \\n => fast loop.\n");
      return true;
    }
      ...
      if ((re_opcode_t) *p1 == charset)
        {
          ...
          for (idx = 0; (idx < (int) p2[1] && idx < CHARSET_BITMAP_SIZE (p1));
               idx++)
...
ptrdiff_t
re_match_2 (struct re_pattern_buffer *bufp, char const *string1,
            ptrdiff_t size1, char const *string2, ptrdiff_t size2,
...
re_match_2_internal (struct re_pattern_buffer *bufp, re_char *string1,
                     ptrdiff_t size1, re_char *string2, ptrdiff_t size2,
                     ptrdiff_t pos, struct re_registers *regs, ptrdiff_t stop)
{
  eassume (0 <= size1);
  eassume (0 <= size2);
  ...
  for (;;)
    {
          ...
          if (d != end_match_2)
            {
...

              {
                ...
                bool same_str_p
                  = (FIRST_STRING_P (match_end) == FIRST_STRING_P (d));
              ...
              /* If no failure points, don't restore garbage.  And if
                ...
                restore_best_regs:
                  ...
                  d = match_end;
                  dend
                    = ((d >= string1 && d <= end1) ? end_match_1 : end_match_2);
      ...
      switch (*p++)
        ...
        case anychar:
          {
            ...
            PREFETCH ();
            buf_ch
              = RE_STRING_CHAR_AND_LENGTH (d, buf_charlen, target_multibyte);
            buf_ch = TRANSLATE (buf_ch);
            if (buf_ch == '\n')
              goto fail;
            ...
            for (;;)
              {
                ...
                /* Compare that many; failure if mismatch, else move
                   past them.  */
                if (!NILP (translate) ? bcmp_translate (d, d2, dcnt, translate,
                                                        target_multibyte)
                                      : memcmp (d, d2, dcnt))
                  {
                    d = dfail;
        ...
        case at_dot:
          ...
          TODO; // if (PTR_BYTE_POS (d) != PT_BYTE)
          //   goto fail;
          break;
        ...
        case notcategoryspec:
          {
...

            {
              ...
              GET_CHAR_AFTER (c, d, len);
              TODO;
              UNUSED (not); // if ((!CHAR_HAS_CATEGORY (c, mcnt)) ^ not)
              //   goto fail;
              d += len;
              nchars++;
            }
...
endof_re_match:
  ...
  if (max_redisplay_ticks > 0 && nchars > 0)
    TODO; //   update_redisplay_ticks (nchars / 50 + 1, NULL);
...
static bool
bcmp_translate (re_char *s1, re_char *s2, ptrdiff_t len, Lisp_Object translate,
                bool target_multibyte)
{
  re_char *p1 = s1, *p2 = s2;
  ...
  while (p1 < p1_end && p2 < p2_end)
    {
      ...
      if (RE_TRANSLATE (translate, p1_ch) != RE_TRANSLATE (translate, p2_ch))
        return true;
                    ...
                    struct re_pattern_buffer *bufp)
{
  ...
  reg_errcode_t ret
    = regex_compile ((re_char *) pattern, length, posix_backtracking,
                     whitespace_regexp, bufp);
