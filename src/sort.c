...
merge_markmem (void *arg)
{
  ...
  if (ms->reloc.size != NULL && *ms->reloc.size > 0)
    {
      Lisp_Object *src
        = (ms->reloc.src->values ? ms->reloc.src->values : ms->reloc.src->keys);
      eassume (src != NULL);
  ...
  if (ms->reloc.order != 0 && *ms->reloc.size > 0)
    {
      Lisp_Object *src
        = (ms->reloc.src->values ? ms->reloc.src->values : ms->reloc.src->keys);
      Lisp_Object *dst
        = (ms->reloc.dst->values ? ms->reloc.dst->values : ms->reloc.dst->keys);
  ...
  if (ms->a.keys == ms->temparray)
    {
      ...
      TODO; //      if (!specpdl_ref_valid_p (ms->count))
            // merge_register_cleanup (ms);
    }
  else
    {
...
static void
merge_lo (merge_state *ms, sortslice ssa, ptrdiff_t na, sortslice ssb,
          ptrdiff_t nb)
{
...
static void
merge_hi (merge_state *ms, sortslice ssa, ptrdiff_t na, sortslice ssb,
          ptrdiff_t nb)
{
  eassume (ms && ssa.keys && ssb.keys && na > 0 && nb > 0);
  eassume (ssa.keys + na == ssb.keys);
...
/* Sort the array SEQ with LENGTH elements in the order determined by
   ...
   optionally reversed.  */
void
tim_sort (Lisp_Object predicate, Lisp_Object keyfunc, Lisp_Object *seq,
          const ptrdiff_t length, bool reverse)
{
